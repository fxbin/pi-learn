...
每章由叙事正文 + 示例代码目录两部分组成：叙事正文写在 `src/content/chapters/sXX_<name>.mdx`（MDX 格式，支持动态图组件），示例代码放在 `public/chapters/sXX_<name>/` 下。示例代码目录必须包含：

1. **code.ts**：完整可运行实现，单文件，`node code.ts` 直接跑
2. **practice.ts**：默写空白页模板，带提示注释的骨架，学习者填肉后 `node practice.ts` 能跑通即过

mdx 正文的结构为：问题 → 解决方案 → 工作原理 → 运行 → 前置概念清单 → 源码锚点 → 妥协清单 → 默写验收。
