...
| cwd 子树校验 | read/write 不做 cwd 子树校验，安全边界交给 `beforeToolCall` 审批钩子 + system prompt | 教学版加 `safePath` 防止写 `/etc/passwd`，真 pi 没有这层；教学场景需要这层保护，但要知道真 pi 的安全模型不是路径校验 |
| 命令黑名单 | bash 工具无黑名单，风险由 prompt + 审批钩子兜底 | 教学版硬编码 `["rm -rf /","sudo",...]`，真 pi 完全没有；教学需要这层护栏，但要知道真 pi 不做字符串黑名单 |
| 超时默认值 | bash schema 无默认超时，模型按需传 timeout | 教学版硬编码 120s，真 pi 可跑半小时长任务；教学需要护栏防止卡死 |
| 输出截断方向 | bash 保留尾部（`truncateTail`），错误信息通常在末尾 | 教学版 `slice(0, N)` 保留头部，会截掉错误堆栈；真 pi 还保存完整输出到临时文件并提示路径 |
