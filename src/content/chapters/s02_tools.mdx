...
[s01](/pi-learn/chapters/s01_agent_loop/) → `s02` → [s03](/pi-learn/chapters/s03_edit_queue/)
...
`resolve` 把相对路径补成绝对路径，`relative` 算出它相对 cwd 的偏移。一旦开头是 `..` 就意味着要跳出 cwd，拒绝。教学版不处理符号链接——pi 用 `realpath` 归一化后再比对。
...
1. `bash.ts` 第 24-27 行用 `typebox` 的 `Type.Object` 定义 schema。pi 为什么不直接写 JSON Schema 对象？（提示：静态类型 + 运行时校验共享一份定义）
2. `path-utils.ts` 的 `resolveReadPathAsync`（在 `read.ts` 第 15 行导入）比写工具的路径解析多走了几步路径变体匹配。为什么读需要这种"宽松"，写不需要？（提示：模型给的路径可能与磁盘大小写/Unicode 不完全一致）
3. `write.ts` 第 203 行用 `withFileMutationQueue` 包裹整个写操作。本章的 write 没有这层包裹——多次并发写同一文件会怎样？（s03 专题解决）

**动手任务（改 mini-pi）**：给 read 加 `offset` 和 `limit` 两个可选参数，参考pi 的 `read.ts` 第 284-324 行：按行切片，offset 是 1-indexed。提示：先用 `split("\n")` 把内容拆成行数组。
...
| cwd 子树校验 | read/write 不做 cwd 子树校验，安全边界交给 `beforeToolCall` 审批钩子 + system prompt | 教学版加 `safePath` 防止写 `/etc/passwd`，pi 没有这层；教学场景需要这层保护，但要知道pi 的安全模型不是路径校验 |
| 命令黑名单 | bash 工具无黑名单，风险由 prompt + 审批钩子兜底 | 教学版硬编码 `["rm -rf /","sudo",...]`，pi 完全没有；教学需要这层护栏，但要知道pi 不做字符串黑名单 |
| 超时默认值 | bash schema 无默认超时，模型按需传 timeout | 教学版硬编码 120s，pi 可跑半小时长任务；教学需要护栏防止卡死 |
| 输出截断方向 | bash 保留尾部（`truncateTail`），错误信息通常在末尾 | 教学版 `slice(0, N)` 保留头部，会截掉错误堆栈；pi 还保存完整输出到临时文件并提示路径 |
| dispatch 表结构 | `ToolDefinition<schema>` 对象，execute 签名带 signal/onUpdate/ctx | 教学版是 `Record<string, (input)=>string>` 同步函数表；s04 的 signal 贯穿、s05 的事件流都依赖对象签名 |
