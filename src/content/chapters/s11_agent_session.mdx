...
mini-pi 的 MiniAgentSession 是教学级简化。真 pi 的实现在 [`packages/coding-agent/src/core/agent-session.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/coding-agent/src/core/agent-session.ts)，先读三段，回答三个问题：

1. `_checkCompaction`（[L1767-1845](https://github.com/earendil-works/pi/blob/v0.76.0/packages/coding-agent/src/core/agent-session.ts#L1767)）的 `skipAbortedCheck` 参数：before-prompt 调用时传 `false`，after-prompt 调用时传 `true`。为什么 before-prompt 要检查 aborted 而 after-prompt 要跳过？（提示：after-prompt 的 assistant 消息如果是 aborted，是谁 abort 的？before-prompt 的上一条 aborted assistant 又是怎么来的？）
...
**动手任务（改 mini-pi）**：给 `_checkCompactionAfter` 的 overflow 路径加一个 `console.warn`——当 `_overflowRecovered` 已经是 true 且再次 overflow 时，打印"上下文溢出恢复失败，请手动压缩或切换更大窗口的模型"。想想：真 pi 在这个分支做了什么（提示：看 L1796-1807 的 `_emit`）？

---

## 妥协清单

mini-pi 的 MiniAgentSession 比真 pi 的 AgentSession 少做了什么，以及为什么省略是安全：
...
| compaction 自动触发 | `AgentHarness` 类是显式 `compact()` 调用，不自动 threshold 触发 | 教学版 `_checkCompactionAfter` 自动检测 threshold 并 compact；真 pi 的自动触发可能在 coding-agent 层（本课程未锚定该层源码），AgentHarness 只提供手动入口 |
| emitHook 错误冒泡 | 真 pi `AgentHarness.emitHook` 错误冒泡（agent-harness.ts 第 244 行），不隔离 | 教学版 `_emit` 只是事件订阅通知，无 hook 概念；与 s10 描述的"三层错误隔离"存在叙事矛盾，真 pi 的 hook 错误是冒泡的 |
| phase 状态机 | 真 pi 有显式 `phase: "idle"/"turn"/"compaction"/"branch_summary"` 字段 | 教学版用 `_isStreaming` + `_lastStopReason` 隐式表达；真 pi 的 `phase !== "idle"` 判断 busy 状态更准确 |
| nextTurnQueue 第三队列 | 真 pi 有第三队列（agent-harness.ts 第 187 行），用于下一轮自动注入 | 教学版只有 steering + followUp 两队列；nextTurnQueue 用于"本轮结束后自动注入"的场景 |
| pendingSessionWrites 延迟写入 | turn 进行中（phase !== "idle"）时 session 写入操作排队，turn 结束才 flush | 教学版无此机制；真 pi 避免 turn 进行中频繁写文件 |

这张表每一行都是真 pi 多花的工程量。读完 `agent-session.ts` 的 constructor 再回头看，你会更清楚每一项在防什么故障。
