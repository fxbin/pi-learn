...
`...` → [s07](/pi-learn/chapters/s07_skills/) → **`s08`** → [s09](/pi-learn/chapters/s09_tui/) → ...
...
| 实现 | 端点 | 数据模型 |
|---|---|---|
| `AnthropicProvider` | `/v1/messages` | Anthropic 原生（content 块数组、tool_use 块、input 对象） |
| `OpenAIProvider` | `/v1/chat/completions` | OpenAI 原生（content 字符串 + tool_calls 数组、arguments JSON 字符串） |

**关键决策：两套供应商数据模型平等独立，pi 抽象出第三套内部模型（AgentMessage）做中间层。** 这不是"一个模型转换成另一个模型"——Anthropic 和 OpenAI 是两套各自完整的数据模型，差异不止 wire 序列化，而是消息结构、工具调用、流式协议、thinking 字段全都不一样。pi 在 `packages/agent/src/types.ts` 定义了不偏向任何供应商的 `AgentMessage` 类型（`toolCall`/`toolResult` 是独立 role，不是 content 块），主循环、工具执行、消息历史全部只跟这套内部模型打交道。

mini-pi 为了减少一层抽象，**选了 Anthropic 形状当内部模型**——这是教学简化，代价是 AnthropicProvider 零转换、OpenAIProvider 看起来像"翻译器"。真实的 pi 里两个 Provider 是平等独立的完整实现，各自调自己的 SDK、解析自己的流式协议，中间隔着一层 AgentMessage 抽象。

这就是**消息边界**：内部模型 ≠ 供应商数据模型，边界处做一次翻译，两边互不污染。pi 的 `transform-messages.ts` 干的就是从 AgentMessage 到供应商 wire 的双向翻译，只是它还要处理 thinking 块、跨供应商 ID 规范化等更复杂的情况。
...
**第 1 块：接口与 AnthropicProvider。** 接口定义了边界契约——`name` 标识身份，`complete` 负责一次 LLM 调用。`AnthropicProvider` 的 `complete()` 与 s01 的 `callLlm` 几乎逐行相同：同样的端点、同样的请求头、同样的 body 结构。注意它是**一份独立的完整实现**——直接调 Anthropic SDK、解析 Anthropic 流式协议、返回 Anthropic 形状的响应，不是"翻译器"。在 pi 里它对应 [`packages/ai/src/providers/anthropic.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/ai/src/providers/anthropic.ts) 的 `streamAnthropic` 函数。

**第 2 块：OpenAIProvider 的独立实现。** 这是本章全部的新东西。`OpenAIProvider` 同样是**一份完整的独立实现**——直接调 OpenAI SDK、解析 OpenAI 流式协议。因为 mini-pi 内部选了 Anthropic 形状，所以 `complete()` 内部要做双向翻译，分两个方向：
...
**Anthropic 后端（默认，也可用 DeepSeek 兼容端点）：**

```bash
# Anthropic 原生：
export ANTHROPIC_API_KEY=sk-ant-...
# 或用 DeepSeek 的 Anthropic 兼容端点：
...
mini-pi 的两个 provider 是教学级简化。pi 的供应商抽象分布在两处，先读再回答问题：

**第一处：[`packages/ai/src/providers/transform-messages.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/ai/src/providers/transform-messages.ts)**
...
读 `detectCompat` 函数（第 1070 行起）。回答：为什么 deepseek 不需要自己的 API 实现，靠 `detectCompat` 里的 `isDeepSeek` 判断就够了？（提示：看第 1099 行的判断条件，以及第 1111-1120 行设置的两个兼容选项）
...
mini-pi 的 provider 比pi 简单很多，以及为什么省略是安全的：

| 省略项 | pi 的做法 | 为什么本章可以省 |
|---|---|---|
| 流式输出 | `StreamFunction` 逐 token 推送，每个供应商一套 stream 解析 | 非流式一次性返回，语义相同，只少了打字机效果 |
| 内部模型独立设计 | pi 有自己的 `AgentMessage` 类型（`toolCall`/`toolResult` 独立 role，不偏向任何供应商），主循环只跟内部模型打交道，供应商实现各自完整 | mini-pi 选 Anthropic 形状当内部模型，省掉了 AgentMessage 中间层抽象，代价是 OpenAIProvider 看起来像"翻译器"而非独立实现 |
...
| provider 抽象范式 | pi 用 `StreamFunction` 类型 + `registerApiProvider` 注册机制 + 懒加载（lazy import），无 `LlmProvider` 接口 | 教学版用 `LlmProvider` 接口 + 两个类实现；pi 的 provider 不是类，是一组导出的 `StreamFunction` |
| deepseek 独立文件 | 无 `providers/deepseek.ts`，deepseek 在 `openai-completions.ts` 的 `detectCompat` 中识别 | 教学版通过 `OPENAI_BASE_URL` 环境变量切换 baseUrl 的方向正确，但pi 的识别逻辑在 openai-completions 内部 |

这张表是 mini-pi 与pi 的差距地图。读完 pi 的 `transform-messages.ts` 再回头看，你会看到每一项省略背后都有一道pi 在防的故障。
...
**供应商抽象到此为止，但 mini-pi 还没走完。** s01-s08 打的是地基：循环（s01）、工具（s02）、编辑（s03）、中断（s04）、会话（s05）、压缩（s06）、技能（s07）、供应商（s08）——八块砖搭起一个能换后端的最小 agent。但 pi 的核心机制还有一半没拆：终端怎么渲染（s09）、扩展怎么挂载（s10）、会话怎么编排（s11）、流式怎么接（s12）、状态怎么保持（s13）、对话怎么分叉（s14）。s08 只是 provider 抽象的入门，s09-s14 才是把 mini-pi 往 pi 方向再推一步的进阶机制。

下一章：[s09 TUI 渲染](/pi-learn/chapters/s09_tui/)
