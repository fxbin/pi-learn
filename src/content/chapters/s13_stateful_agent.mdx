...
mini-pi 的 MiniAgent，在pi 里是 [`packages/agent/src/agent.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/agent/src/agent.ts) 的 `Agent` 类。配套双层循环在 [`packages/agent/src/agent-loop.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/agent/src/agent-loop.ts)。读完回答三个问题：
...
下一章：[s14 会话分支树 — 一个 .jsonl 如何承载对话分叉](/pi-learn/chapters/s14_session_tree/)
