...
mini-pi 的 skills 是教学级简化。真 pi 的实现在 [`packages/agent/src/harness/skills.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/agent/src/harness/skills.ts)，先读这个文件，回答三个问题（答案就在代码里）：
...
mini-pi 的 skills 比真 pi 简单很多，以及为什么省略是安全：
...
| SKILL.md 优先级 | 目录下优先找 SKILL.md，找到就只加载它，跳过其他 .md | 教学版对所有 .md 文件都加载，无优先级；真 pi 的 SKILL.md 是"入口文件"语义 |
| description 空丢弃 | `loadSkillFromFile` 第 265-267 行，description 为空返回 `skill:null` 丢弃 | 教学版不要求 description；真 pi 会丢弃无描述的 skill（但仍记录 diagnostic） |
