...
| 启动前 drain steering | 真 pi 在循环开始前（agent-loop.ts 第 167 行）先调一次 getSteeringMessages | 教学版只在每轮开头 drain，会漏掉"agent 启动到首次调 LLM 之间"用户敲的字 |
