...
mini-pi 的单层循环，在pi 里是 `runLoop`：[`packages/agent/src/agent-loop.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/agent/src/agent-loop.ts) 第 155-269 行。配套类型在 [`packages/agent/src/types.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/agent/src/types.ts)（搜 `getSteeringMessages`、`getFollowUpMessages`）。读完回答三个问题：
