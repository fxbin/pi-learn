...
mini TUI 的两百行引擎，在真 pi 里对应 [`packages/tui/src/tui.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/tui/src/tui.ts)。先读一遍对应行号，再回答三个问题（答案就在代码里）：
...
**动手任务（改 mini TUI）**：真 pi 的 `scheduleRender` 在 `doRender` 结束后检查 `renderRequested`，如果渲染期间又有新请求就继续调度。mini TUI 没有这个循环。给 `scheduleRender` 的 `setTimeout` 回调末尾加一句：`if (this.renderRequested) this.scheduleRender()`。验证：连续快速按 ↑ 五次，观察是否合并成一帧而不是五帧。

---

## 妥协清单

mini TUI 比真 pi 少做了什么，以及为什么省略是安全的：
...
| scheduleRender 自循环 | 真 pi `scheduleRender` 回调末尾检查 `renderRequested`，有新请求则继续调度（tui.ts 第 538-540 行） | 教学版回调末尾不检查，连续高频刷新会丢帧；单帧场景不受影响 |
| requestRender(force) | 真 pi 支持 `force` 参数强制清屏重画（置 `previousWidth=-1`），用于主题切换等场景 | 教学版无 force 参数，无法强制重画；主题切换等场景需要 |

这张表是 mini TUI 与真 pi 的差距地图：真 pi 在这套差分骨架上叠了图像、叠层、IME、滚动、移动端兼容，但核心差分逻辑就是本章这三块。
