...
```
   ...
   └──────┬──────┘
          ▼
   stop_reason ≠ tool_use ? ──► 正常结束
          │
          ▼
   执行工具 → turns++ → 超 maxTurns ? ──► 熔断
          │
          └──► 下一轮
```
...
mini-pi 的单层循环，在真 pi 里是 `runLoop`：`.reference/pi/packages/agent/src/agent-loop.ts` 第 155-275 行。配套类型在 `packages/agent/src/types.ts`（搜 `getSteeringMessages`、`getFollowUpMessages`）。读完回答三个问题：
...
| signal 贯穿工具 | 工具执行也收 signal，长任务可被中断 | 本章 bash 用 `spawnSync` 同步阻塞，signal 在执行中无法响应；s02 换异步工具后才有意义 |
