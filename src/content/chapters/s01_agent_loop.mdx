...
`s01` → [s02](/pi-learn/chapters/s02_tools/)
...
mini-pi 的十几行循环，在pi 里对应 `runLoop`：[`packages/agent/src/agent-loop.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/agent/src/agent-loop.ts) 第 155-269 行。先读一遍，再回答两个问题（答案就在代码里）：
