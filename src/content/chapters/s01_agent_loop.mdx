...
`stop_reason` 只用于异常检测，不用于判断循环继续——这是与真 pi 一致的设计。
...
**第 3 块：主循环。** 全章核心，只有十几行。退出判断与真 pi 一致——看 content 里有没有 tool_use 块：
...
mini-pi 的十几行循环，在真 pi 里对应 `runLoop`：[`packages/agent/src/agent-loop.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/agent/src/agent-loop.ts) 第 155-275 行。先读一遍，再回答三个问题（答案就在代码里）：
...
**关于退出判断**：mini-pi 和真 pi 都用"`content` 里有没有 `tool_use` 块"判断循环继续，`stop_reason` 只用于异常检测（`"error"`/`"aborted"`）。不要用 `stop_reason === "tool_use"` 判断——那是 Anthropic 的命名，换到 OpenAI 就叫 `"tool_calls"`，写死会绑死 provider。
