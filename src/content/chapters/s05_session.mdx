...
mini-pi 的两个函数，在真 pi 里对应一整套 session 子系统：[`packages/agent/src/harness/session/`](https://github.com/earendil-works/pi/tree/v0.75.5/packages/agent/src/harness/session/)。先读 [`jsonl-storage.ts`](https://github.com/earendil-works/pi/blob/v0.75.5/packages/agent/src/harness/session/jsonl-storage.ts)，再回答三个问题：
