...
mini-pi 的两个函数，在真 pi 里对应一整套 session 子系统：[`packages/agent/src/harness/session/`](https://github.com/earendil-works/pi/tree/v0.76.0/packages/agent/src/harness/session/)。先读 [`jsonl-storage.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/agent/src/harness/session/jsonl-storage.ts)，再回答三个问题：
...
| 持久化格式与 LLM 格式分离 | SessionTreeEntry（含 message/compaction/label 等多种类型）与 AgentMessage[] 不同，需 `buildSessionContext` 转换 | 教学版直接持久化 LLM 格式，文件里存的 = 模型看到的；真 pi 的转换在 s06/s14 显现 |
