...
| 文件 | 关键行号 | 看什么 |
|---|---|---|
| [`.../extensions/runner.ts`](https://github.com/earendil-works/pi/blob/v0.75.5/packages/coding-agent/src/core/extensions/runner.ts) | 680-712 | 通用 `emit`：try/catch 错误隔离 + session_before 事件的 cancel 短路 |
...
| [`.../extensions/loader.ts`](https://github.com/earendil-works/pi/blob/v0.75.5/packages/coding-agent/src/core/extensions/loader.ts) | 124-170 | `createExtensionRuntime`：十几个 action 方法全是 throwing stubs，等 bindCore 替换 |
| 同上 | 331-343 | `loadExtensionModule`：jiti 运行时加载 TS，取 default 作为 factory |
| [`.../extensions/types.ts`](https://github.com/earendil-works/pi/blob/v0.75.5/packages/coding-agent/src/core/extensions/types.ts) | 949-972 | `ExtensionEvent` 联合类型：26 个事件 |
