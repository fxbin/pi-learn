...
concepts:
  ...
  - ToolUseBlock.input
---
import MutationQueueDiagram from "../../components/diagrams/MutationQueueDiagram.tsx";

# s03: edit + 变更队列 — 字符串手术与写盘串行化

> edit 是最难的工具：它修改已有内容，而不是整文件覆盖。

[s02](/pi-learn/chapters/s02_tools/) → `s03` → [s04](/pi-learn/chapters/s04_interrupt/)
...
<MutationQueueDiagram client:load />
...
mini-pi 的 edit + queue，在pi 里对应三个文件：

1. [`packages/coding-agent/src/core/tools/edit.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/coding-agent/src/core/tools/edit.ts) — edit 工具入口，`withFileMutationQueue` 包裹整个读-改-写
2. [`packages/coding-agent/src/core/tools/edit-diff.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/coding-agent/src/core/tools/edit-diff.ts) — 模糊匹配、BOM 处理、diff 生成
3. [`packages/coding-agent/src/core/tools/file-mutation-queue.ts`](https://github.com/earendil-works/pi/blob/v0.76.0/packages/coding-agent/src/core/tools/file-mutation-queue.ts) — 按文件 canonical path 分 key 的队列

先读一遍，再回答三个问题（答案就在代码里）：

1. `file-mutation-queue.ts` 用 `Map<string, Promise<void>>` 按 canonical path 分锁（key 是 realpath 归一化后的 canonical path）。为什么按 path 分锁而不是全局单队列？（提示：不同文件的编辑互不阻塞）
2. `edit-diff.ts` 的 `fuzzyFindText` 先精确匹配，再走 `normalizeForFuzzyMatch`。什么情况下精确匹配失败但模糊匹配成功？（提示：看 normalize 做了哪些归一化）
3. `edit.ts` 第 344-350 行：stripBom → detectLineEnding → normalizeToLF → applyEditsToNormalizedContent → restoreLineEndings。为什么不在原始字节上直接替换？（提示：CRLF 文件里 old_string 写的是 LF）
...
| 按 path 分 key | `Map<string, Promise<void>>`（key 是 realpath 归一化后的 canonical path，不是 ExecutionEnv） | 全局单队列，教学够用；pi 按 canonical path 分锁提升并发 |
| cwd 子树校验 | edit 不做 cwd 子树校验，同 s02 | 教学版复用 `safePath`，pi 没有；详见 s02 妥协清单 |
