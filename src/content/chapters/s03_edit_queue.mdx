...
1. [`packages/agent/src/harness/tools/edit.ts`](https://github.com/earendil-works/pi/blob/v0.75.5/packages/agent/src/harness/tools/edit.ts) — edit 工具入口，`withFileMutationQueue` 包裹整个读-改-写
2. [`packages/agent/src/harness/tools/edit-diff.ts`](https://github.com/earendil-works/pi/blob/v0.75.5/packages/agent/src/harness/tools/edit-diff.ts) — 模糊匹配、BOM 处理、diff 生成
3. [`packages/agent/src/harness/tools/file-mutation-queue.ts`](https://github.com/earendil-works/pi/blob/v0.75.5/packages/agent/src/harness/tools/file-mutation-queue.ts) — 按文件 canonical path 分 key 的队列
