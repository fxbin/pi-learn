...
| 按 path 分 key | `Map<string, Promise<void>>`（key 是 realpath 归一化后的 canonical path，不是 ExecutionEnv） | 全局单队列，教学够用；真 pi 按 canonical path 分锁提升并发 |
| cwd 子树校验 | edit 不做 cwd 子树校验，同 s02 | 教学版复用 `safePath`，真 pi 没有；详见 s02 妥协清单 |
