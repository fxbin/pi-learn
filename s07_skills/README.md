...
`...` → [s01](../s01_agent_loop/) → ... → [s06](../s06_compaction/) → **`s07`** → [s08 provider 抽象](../s08_provider/)
...
打开 [code.ts](code.ts)，重点看三块，其余是 s01 原样。
...
合上 code.ts 和本 README，打开 [practice.ts](practice.ts)，凭记忆补全三个新函数：`loadSkills`、`parseSkill`、`buildSystemPrompt`，以及改写 `main`（启动时加载 skills、构建 system、传给 agentLoop）。`callLlm`、`runBash`、`agentLoop` 是 s01 原样稍改（多传一个 system 参数），写不出可以查。
...
下一章：[s08 provider 抽象](../s08_provider/)
