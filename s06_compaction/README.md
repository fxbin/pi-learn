...
`...` → [s01](../s01_agent_loop/) → ... → **`s06`** → [s07](../s07_skills/)
...
打开 [code.ts](code.ts)，重点看三块新东西，其余是 s01 原样。
...
合上 code.ts 和本 README，打开 [practice.ts](practice.ts)，凭记忆补全两个新函数：`estimateTokens`、`compact`，以及改写 `agentLoop`（在循环开头插入压缩闸门）。`callLlm`、`runBash`、`main` 是 s01 原样，写不出可以查。
